...
<head>
    ...
    <title>Find Internships - JobSure</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .nav-links a[href*="internships.php"] { color: var(--primary-dark); font-weight: 600; }
        ...
        @media (max-width: 900px) {
            ...
            .nav-right { gap: 10px; }
            /* Mobile button fixes (if any were present) */
...
<body>
    ...
    <main class="container">
        <div class="page-content-wrapper">
            <h1 class="page-title">Find Your Next Internship</h1>

            <form id="filter-form">
                <aside class="filter-bar">
                    <div class="filter-group filter-search">
                        <label for="filter-search" class="filter-label">Search</label>
                        <input type="text" id="filter-search" name="search" class="filter-search-input" placeholder="Internship title or company...">
                    </div>
                    ...
                    <div class="filter-group">
                        <label for="filter-stipend" class="filter-label">Stipend Range</label>
                        <select id="filter-stipend" name="stipend" class="filter-select">
                    ...
                    <div class="filter-group">
                        ...
                        <select id="filter-duration" name="duration" class="filter-select">
            ...
            <section class="job-listings" id="job-listings-container">
                <a href="#" class="job-card-link">
                    <div class="job-card-header">
                         <p style="text-align: center; color: var(--text-light);">Loading internships...</p>
                    </div>
                </a>
            </section>
        </div>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function createInternshipCard(internship) {
                ...
                if (internship.skills) {
                    internship.skills.split(',').forEach(skill => {
                        if (skill.trim()) tagsHTML += `<span class="tag">${skill.trim()}</span>`;
                    });
                }
                const logoInitial = internship.company_name ? internship.company_name.substring(0, 2).toUpperCase() : '??';

                return `
                    <a href="view_listing.php?id=${internship.id}" class="job-card-link">
                        <div class="job-card-header">
                            <div class="job-card-title-wrap">
                                <span class="job-card-title">${internship.job_title}</span>
                                <div class="job-card-company">${internship.company_name}</div>
                            </div>
                            <div class="job-card-logo">${logoInitial}</div>
                        </div>
                        <div class="job-card-details">
                            ${internship.location ? `<span class="detail-item"><i data-lucide="map-pin"></i> ${internship.location}</span>` : ''}
                            ${internship.pay_details ? `<span class="detail-item"><i data-lucide="wallet-2"></i> ${internship.pay_details}</span>` : ''}
                            ${internship.duration ? `<span class="detail-item"><i data-lucide="calendar"></i> ${internship.duration}</span>` : ''}
                        </div>
            ...
            async function fetchListings() {
                ...
                try {
                    const response = await fetch(`api_fetch_internships.php?${params.toString()}`);
                    ...
                    listings.forEach(listing => {
                        const cardHTML = createInternshipCard(listing);
